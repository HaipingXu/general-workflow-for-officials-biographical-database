...
def run_export(province: str, province_full: str, data_subdir: str = "",
               drop_study: bool = False) -> dict:
...
def run_province_pipeline(
    ...
    if not skip_extract:
        if not skip_battle:
            ...
            t_ph = time.time(); tok_ph = TOKENS.snapshot()
            if skip_step1:
                print(f"\n☆☆ Phase A: [跳过 Step1] 仅运行 Step4 (使用已存在 merged_episodes_step1.json) ☆☆")
...
示例:
  ...
  python main_province.py --province 安徽 --force          # 强制重跑
    ...
    parser.add_argument("--skip-step1", action="store_true", help="跳过Step1提取（仅运行Step2/3/4，需要 merged_episodes_step1.json 已存在）")
    ...
    parser.add_argument("--drop-study", action="store_true",
                        help="导出时删除学习/进修经历（标志位=学习进修），并按人重排经历序号。默认关闭")
