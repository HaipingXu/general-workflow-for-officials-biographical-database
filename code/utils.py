...
def _find_top_level_json_blocks(text: str) -> list[str]:
    """Find all top-level {...} JSON object blocks via brace counting.

    Handles strings (including escaped quotes) so that braces inside string
    literals don't break tracking. Returns ordered list of candidate blocks.
    """
    blocks = []
    depth = 0
    start = -1
...
def extract_json(text: str) -> dict:
    """
    Clean up LLM output and parse as JSON dict.

    Robust against Claude's occasional "double JSON + self-revision" pattern:
    {first version} ...explanation... {revised version}
    We pick the LAST valid JSON block (Claude's final answer).

    Falls back to json_repair for malformed JSON (missing commas, etc.).
    """
    text = re.sub(r"^```(?:json)?\s*", "", text.strip())
    text = re.sub(r"\s*```$", "", text.strip())

    # Fast path: whole text is one clean JSON
    try:
    ...
    # Multi-block path: extract all top-level {...} blocks, try parsing each
    # in reverse order (last = Claude's most-refined answer).
    blocks = _find_top_level_json_blocks(text)
    if len(blocks) > 1:
        logger.warning(
            f"[JSON extract] found {len(blocks)} top-level JSON blocks, "
            f"using last (Claude self-revision pattern); head={text[:120]!r}"
        )
    ...
    # Repair path: try json_repair on the last block (or whole text)
...
_NON_RETRYABLE_PATTERNS = (
    ...
    "sensitive_words",
    "risk_content",
    # Cost-killer patterns (added after retry-storm burned ~$120 in 4 provinces):
    "contain the word 'json'",        # OpenAI strict 400, won't recover via retry
    "预扣费额度失败",                  # Clariontech 403 balance exhausted
    "insufficient_user_quota",         # Clariontech / OpenAI quota exhausted
    "model_not_found",                 # 503 wrong model name
)
...
def llm_chat(
    ...
    for attempt in range(max_retries + 1):
        ...
        # OpenAI strict validation: when response_format=json_object is set,
        # the USER message specifically must contain the literal word "json"
        # (case-insensitive). System prompt does NOT count. Clariontech started
        # enforcing this after HTTPS upgrade. Auto-append a marker if user
        # doesn't already mention json.
        effective_user = user
        if (response_format and response_format.get("type") == "json_object"
                and "json" not in user.lower()):
            effective_user = user + "\n\n(请以 JSON 格式回复 / reply in JSON)"
        ...
        kwargs = {
            ...
            "stream": stream,
            # Disable tool calling: prevents Claude (via Clariontech proxy) from
            # auto-invoking phantom tools like "EnterPlanMode" and returning
            # empty content with finish_reason="tool_calls".
            "tool_choice": "none",
        }
