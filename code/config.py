...
CLARIONTECH_BASE_URL = "https://arkvision.ai/v1"
...
# --- LLM2: Verifier (Claude Opus 4.7 via Clariontech NewAPI) ---
# Switched from gpt-5.2 to claude-opus-4-7: gpt-5.2 channel on Clariontech
# suffered persistent 504s on long-prompt step2 extractions (367 504s across
# 6 runs, all attributed to gpt-5.2). claude-opus-4-7 is healthy on the same
# proxy and gives us true heterogeneous double-verification vs LLM1 (DeepSeek).
LLM2_API_KEY = CLARIONTECH_API_KEY
LLM2_API_KEYS = CLARIONTECH_API_KEYS
LLM2_BASE_URL = CLARIONTECH_BASE_URL
LLM2_MODEL = "claude-opus-4-7"

# --- Judge: GPT-5.5 (via Clariontech) ---
JUDGE_API_KEY = CLARIONTECH_API_KEY
JUDGE_API_KEYS = CLARIONTECH_API_KEYS
JUDGE_BASE_URL = CLARIONTECH_BASE_URL
JUDGE_MODEL = "gpt-5.5"  # 从 gpt-5.4 切换：5.4 在 arkvision 源频繁 524/empty，5.5 探针更快更稳
...
# --- Paths ---
# 代码位于 code/ 子目录；项目根目录是其上一级。
# 输入(data/, officials/)、prompts/、产物(output/, logs/) 均在根目录，便于组员维护。
...
# 标志位 marking a 脱产学习/进修 episode (本科生/研究生/进修学员/访问学者/留学).
# Used by the optional "drop study experiences on export" feature (export.run_export
# drop_study=True). Single source of truth for that filter.
STUDY_FLAG = "学习进修"
...
LLM2_MAX_WORKERS = 40      # Verifier (Clariontech)
JUDGE_MAX_WORKERS = 40     # Judge (Clariontech)
SCRAPE_WORKERS = 2         # Scraping parallelism (low to avoid anti-bot detection)


# --- Retry / Backoff (used by utils.llm_chat) ---
# max_retries=3 (was 10 — too aggressive, retry storms when balance hit zero
# wasted ~$120 in input tokens. Use NON_RETRYABLE_PATTERNS for fast-fail on
# deterministic errors like quota exhaustion / json keyword 400.)
LLM1_MAX_RETRIES = 3
LLM2_MAX_RETRIES = 3
JUDGE_MAX_RETRIES = 3

# 429 backoff: wait = BASE * (FACTOR ** attempt) + jitter   →  20s, 60s, 180s, ...
LLM_429_BACKOFF_BASE = 20.0
LLM_429_BACKOFF_FACTOR = 3.0
LLM_429_BACKOFF_CAP = 360.0   # absolute ceiling per attempt

# Generic (5xx / network) backoff: wait = BASE * (attempt+1) + jitter
LLM_GENERIC_BACKOFF_BASE = 6.0
...
LLM2_PROVIDER_CONCURRENCY = 40
JUDGE_PROVIDER_CONCURRENCY = 40
